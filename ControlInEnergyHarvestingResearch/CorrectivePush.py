...
for k in pointslist:
    ...
    totPoints = 1000000*factor #Number of simulated points
    periods = 1000*factor #Number of simulated periods
    ...
    # Absolute value of average force
    ...
    phase.set_title('a)                 g = {:.5f}'.format(gOr), loc = 'left')
    ...
    #
    LineGraphF = plt.subplot(col,row,4)
    ...
    # plt.xlabel('v')
    # plt.ylabel(r'$P_v$')
    LineGraphF.set_title('d)         Force v. Time for g = {:.5f}'.format(gOr), loc = 'left')
...
for j in testList:
    ...
    # plt.axis([-1.75,1.75,-1, 1])
    if(loop == 1):
        phase.set_title('b)                        g = {:.5f}'.format(gOr), loc = 'left')
    else:
